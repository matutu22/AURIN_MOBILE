...
open class Legend: ComponentBase
{
    ...
    open func getMaximumEntrySize(withFont font: NSUIFont) -> CGSize
    {
        ...
        for entry in entries
        {
            ...
            let size = (label as NSString!).size(withAttributes: [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): font])
    ...
    open func calculateDimensions(labelFont: NSUIFont, viewPortHandler: ViewPortHandler)
    {
        ...
        case .vertical:
            ...
            for i in 0 ..< entryCount
            {
                ...
                if label != nil
                {
                    let size = (label as NSString!).size(withAttributes: [NSAttributedStringKey(rawValue: NSAttributedStringKey.font.rawValue): labelFont])
